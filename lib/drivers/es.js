...
ElasticSearchDriver.prototype.readMapping = function(cb) {
  ...
  this.get([this.opts.index, this.opts.type, '_mapping'], function(err, mapping) {
    if(err || self.opts.type) return cb(err, mapping);
...
ElasticSearchDriver.prototype.request = function(method, path_parts, data, cb) {
  ...
  if(this.opts.verbose) {
    ...
    if(debug_str) {
      if(typeof debug_str != "string") debug_str = JSON.stringify(debug_str);
      debug_str = "[" + debug_str.length + "] => " + debug_str.slice(0,100).replace(/[\n]/g, '\\n') + (debug_str.length > 100 ? "..." : "");
    }

    console.log(opts.method.red, opts.url.green, debug_str ? debug_str.grey.italic : '');
  }
...
ElasticSearchDriver.prototype.head = function(path_parts, cb) {
  this.request('HEAD', path_parts, cb);
}
